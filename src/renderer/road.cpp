...
void Road::UpdateVertices(glm::vec3 point_a, glm::vec3 point_b)
{
    ...
    // Vertical lines to mark the two points
    verts.insert(verts.end(), {point_a.x, point_a.y, point_a.z});
    verts.insert(verts.end(), {point_a.x, point_a.y+5, point_a.z});

    verts.insert(verts.end(), {point_b.x, point_b.y, point_b.z});
    verts.insert(verts.end(), {point_b.x, point_b.y+5, point_b.z});

    const float width = 3.0f;
    const float radius = width/2;
    const int numberOfSides = 8; // Must be multiple of 4
...
void Road::Draw(glm::mat4 view, glm::mat4 projection)
{
    ...
    glLineWidth(5.0f);
    glBindVertexArray(VAO);
    glDrawArrays(GL_LINES, 0, 6); // Draw vertical lines

    // Draw both semi-circles
    // GL_TRIANGLE_FAN draws n-2 for n
    glDrawArrays(GL_TRIANGLE_FAN, 4, roadCurveSides/2 + 2); // Strips
    glDrawArrays(GL_TRIANGLE_FAN, roadCurveSides/2 + 6, roadCurveSides/2 + 2); // Strips 2

    // Draw the 3 box parts of the road
    // n-2 so we need 8 verts for 6 triangles
    glDrawArrays(GL_TRIANGLE_STRIP, roadCurveSides + 8 , 8);
